...
SELECT id, u.created_at, u.updated_at, email, hashed_password, token, r.created_at, r.updated_at, expires_at, revoked_at, user_id FROM users u INNER JOIN refresh_tokens r ON u.id = r.user_id WHERE r.token = $1
`
